...
from ..consts import FCNET_TASKS
import os
import numpy as np

class FCNetTransferExperiment(TransferBenchmarkExperiment):
    ...
    def __init__(self, optimizer_name, prior_tasks, target_task, seed=0, 
        ...
        if len(self.prior_tasks) == 1 and self.prior_tasks[0] == 'all':
            self.prior_tasks = [t for t in FCNET_TASKS if t != target_task]
